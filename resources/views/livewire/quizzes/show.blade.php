        ...
        <div class="flex items-start justify-between mb-4">
            <div class="flex-1">
                ...
                @if($assessment->description)
                    <p class="text-indigo-100 mb-4">{{ $assessment->description }}</p>
                @endif
                <div class="flex items-center gap-6 text-sm flex-wrap">
                    <div class="flex items-center gap-2">
                        ...
                        <span>{{ $assessment->lesson->course->title ?? 'N/A' }}</span>
                    </div>
        ...
        @if($hasTaken && $bestAttempt)
            <div class="mt-6 bg-white/20 rounded-lg p-4">
                ...
                <div class="w-full bg-white/20 rounded-full h-2">
                    <div class="h-2 {{ $bestScore >= $assessment->passing_score ? 'bg-green-300' : 'bg-red-300' }} rounded-full transition-all duration-300" 
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            @if($assessment->time_limit_minutes)
                <div>
                    ...
                    <p class="text-lg font-semibold text-gray-900 dark:text-white">{{ $assessment->time_limit_minutes }} minutes</p>
                </div>
            @endif
            @if($assessment->passing_score)
                <div>
                    <p class="text-sm text-gray-600 dark:text-gray-400">Passing Score</p>
                    <p class="text-lg font-semibold text-gray-900 dark:text-white">{{ $assessment->passing_score }}%</p>
                </div>
            @endif
            @if($assessment->max_attempts)
                <div>
                    <p class="text-sm text-gray-600 dark:text-gray-400">Max Attempts</p>
                    <p class="text-lg font-semibold text-gray-900 dark:text-white">{{ $assessment->max_attempts }}</p>
                </div>
            @else
                <div>
                    <p class="text-sm text-gray-600 dark:text-gray-400">Max Attempts</p>
            ...
            <div>
                ...
                <p class="text-lg font-semibold text-gray-900 dark:text-white">{{ $assessment->is_randomized ? 'Yes' : 'No' }}</p>
            </div>
            <div>
                <p class="text-sm text-gray-600 dark:text-gray-400">Show Answers</p>
                <p class="text-lg font-semibold text-gray-900 dark:text-white">{{ $assessment->show_correct_answers ? 'Yes' : 'No' }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="flex items-center gap-4">
        ...
        @else
            @php
                ...
                if ($assessment->max_attempts && $attemptsRemaining !== 'unlimited' && $attemptsRemaining <= 0) {
                    $canTakeQuiz = false;
            ...
            @if($incompleteAttempt)
                <flux:button href="{{ route('quizzes.take', $assessment) }}" variant="primary" class="flex-1 bg-gradient-to-r from-indigo-500 to-purple-600" wire:navigate>
        ...
        @can('update', $assessment)
            <flux:button href="{{ route('quizzes.edit', $assessment) }}" variant="ghost" wire:navigate>
                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
