...
@if(!$isInstructor && $lesson->is_locked)
    @include('livewire.lessons.partials.locked')
@else
@if(!$isInstructor)
                ...
                <div x-show="open" x-transition:enter="transition-all duration-200 ease-out" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100">
                    ...
                    >
                        ...
                        <span class="flex-shrink-0 w-4 h-4">
                            @if(!$isInstructor && ($lessonItem->is_locked ?? false))
                                <svg class="w-4 h-4 text-amber-500" fill="currentColor" viewBox="0 0 20 20" title="Locked">
