<div class="min-h-screen bg-slate-50 dark:bg-zinc-950">
            ...
            <div class="flex items-center justify-between gap-4">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-orange-100">{{ $currentUser->name }}</p>
                    </div>
                </div>
                <a href="{{ route('admin.users.show', $currentUser) }}" wire:navigate
    ...
    <div class="mx-auto max-w-3xl px-4 py-5 sm:px-6 space-y-4">
        ...
        <form wire:submit.prevent="update" class="space-y-4">
            ...
            <div class="rounded-lg border border-slate-200 bg-white dark:border-zinc-700 dark:bg-zinc-900">
                ...
                <div class="space-y-4 p-4">
                    @if($currentUser->profile_image)
                        <div class="flex items-center gap-3">
                            <img src="{{ Storage::url($currentUser->profile_image) }}" alt=""
                                 class="h-12 w-12 rounded-full border border-slate-200 object-cover dark:border-zinc-700">
                            <p class="text-xs text-slate-500">Upload a new image to replace the current one</p>
                        </div>
                    @endif
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <flux:input wire:model="user.name" label="Full name" required />
                        <flux:input wire:model="user.email" type="email" label="Email" required />
                    </div>
                    <flux:textarea wire:model="user.bio" label="Bio" rows="2" />
                    <div>
            ...
            <div class="rounded-lg border border-slate-200 bg-white dark:border-zinc-700 dark:bg-zinc-900">
                <div class="border-b border-slate-100 px-4 py-3 dark:border-zinc-800">
                    <h2 class="text-xs font-bold uppercase tracking-wide text-slate-500">Change password</h2>
                    <p class="mt-0.5 text-xs text-slate-400">Leave blank to keep the current password</p>
                </div>
