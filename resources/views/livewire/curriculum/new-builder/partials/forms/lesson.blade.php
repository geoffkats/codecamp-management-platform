    ...
    <form wire:submit.prevent="saveLesson" class="flex flex-col h-full">
        ...
        <div class="flex-1 overflow-y-auto">
            <div class="max-w-3xl mx-auto px-6 py-8 space-y-6">
                ...
                <div x-data="{ previewUrl: null, previewName: null }" class="space-y-4">
                <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden">
                    ...
                    <div class="flex items-center gap-3 px-5 py-4 border-b border-gray-100 dark:border-gray-800">
                        ...
                        <div class="flex-1 min-w-0">
                            <p class="text-sm font-bold text-gray-900 dark:text-white">Lesson Slides</p>
                            <p class="text-xs text-gray-500 dark:text-gray-400">PDF becomes the main lesson content students see</p>
                        </div>
                        {{-- Current file badge --}}
                        @if(!empty($formData['slide_file_path']))
                            <span class="flex-shrink-0 inline-flex items-center gap-1.5 px-2.5 py-1 bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300 text-xs font-semibold rounded-full">
                    ...
                    <div class="px-5 py-4 space-y-3">
                        {{-- Current file row (if saved) --}}
                        @if(!empty($formData['slide_file_path']))
                            @php $slideFilename = basename($formData['slide_file_path']); @endphp
                            <div class="flex items-center gap-3 px-3 py-2.5 bg-gray-50 dark:bg-gray-800 rounded-lg">
                                ...
                                <span class="flex-1 text-sm text-gray-700 dark:text-gray-300 truncate font-medium">{{ $slideFilename }}</span>
                                @if($selectedId)
                                    <button type="button" wire:click="removeSlideFile"
                                            wire:confirm="Remove this slide file? Students will see no content until you upload a new one."
                        ...
                        <div>
                            <input type="file"
                                   ...
                                   class="block w-full text-sm text-gray-600 dark:text-gray-400
                                          ...
                                          cursor-pointer">
                            <p class="mt-1.5 text-xs text-gray-400 dark:text-gray-500">PDF only · Max 50 MB · Export PowerPoint as PDF first</p>
                        </div>

                        <div wire:loading wire:target="slideUpload" class="flex items-center gap-2 text-sm text-orange-600 dark:text-orange-400">
                        ...
                        @error('slideUpload') <p class="text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                    </div>
                </div>{{-- /upload card --}}

                {{-- ── PDF Preview Panel (separate, full-width) ──────────────── --}}
                    ...
                    <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden">
                        <div class="flex items-center justify-between px-5 py-3 border-b border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/50">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-sm font-semibold text-gray-700 dark:text-gray-300">Slide Preview</span>
                            </div>
                            <a href="{{ \Illuminate\Support\Facades\Storage::disk('public')->url($formData['slide_file_path']) }}"
                               target="_blank"
                               class="text-xs font-semibold text-orange-600 dark:text-orange-400 hover:underline">
                                Open full screen ↗
                            </a>
                        </div>
                        <iframe src="{{ \Illuminate\Support\Facades\Storage::disk('public')->url($formData['slide_file_path']) }}"
                                class="w-full border-0 block"
                                style="height: 680px;"
                                title="Current lesson slides"></iframe>
                    </div>
                @endif

                {{-- New file preview (client-side blob, before saving) --}}
                <div x-show="previewUrl" x-cloak
                     class="bg-white dark:bg-gray-900 rounded-xl border border-orange-200 dark:border-orange-800 overflow-hidden">
                    <div class="flex items-center justify-between px-5 py-3 border-b border-orange-100 dark:border-orange-800 bg-orange-50 dark:bg-orange-900/20">
                        <div class="flex items-center gap-2">
                            <svg class="w-4 h-4 text-orange-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                            </svg>
                            <span class="text-sm font-semibold text-orange-800 dark:text-orange-200">New file preview</span>
                            <span class="text-xs text-orange-600 dark:text-orange-400" x-text="'— ' + previewName"></span>
                        </div>
                        <span class="text-xs font-semibold text-orange-600 dark:text-orange-400 bg-orange-100 dark:bg-orange-900/40 px-2 py-0.5 rounded-full">
                            Unsaved
                        </span>
                    </div>
                    <iframe x-bind:src="previewUrl"
                            class="w-full border-0 block"
                            style="height: 680px;"
                            title="New slide preview"></iframe>
                </div>
                </div>{{-- /x-data wrapper --}}
